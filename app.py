...
def process_update(update):
    if 'callback_query' in update:
        ...
        if data == 'menu_main': show_main_menu(chat_id, message_id)
        ...
        elif data == 'menu_favorites': show_favorites(chat_id, message_id, user_id)
        elif data == 'menu_help': show_help(chat_id, message_id)
        elif data == 'search_track': show_search_prompt(chat_id, message_id, 'track', 'menu_search', user_id)
        elif data == 'search_artist': show_search_prompt(chat_id, message_id, 'artist', 'menu_search', user_id)
        elif data == 'create_playlist': show_create_playlist_prompt(chat_id, message_id, user_id)
        elif data.startswith('playlist_'):
            parts = data.split('_')
            show_playlist_tracks(chat_id, message_id, int(parts[1]), int(parts[2]) if len(parts) > 2 else 0)
            ...
            if track:
                try:
                    cursor.execute('INSERT INTO playlist_tracks (playlist_id, video_id, title, artist) VALUES (?, ?, ?, ?)', (pl_id, video_id, track[0], track[1]))
                    conn.commit()
            ...
            if track:
                try:
                    cursor.execute('INSERT INTO favorites (user_id, video_id, title, artist) VALUES (?, ?, ?, ?)', (user_id, video_id, track[0], track[1]))
                    conn.commit()
